...
* `desc` - description, entities decoded, whitespace collapsed, at most 400 characters. Omitted when empty.
...
cd panthers-video-index
git show gh-pages:videos.json > videos.json      # start from the published index
...
|-----------------------|---------------|---------|
| `--existing PATH`     | `videos.json` | Index to merge into (v1 or v2). Missing file = build from scratch. |
...
2. Parses every `<url>` block as a unit (title, description, thumbnail, date, duration and tags
   ...
   ~96% of descriptions). Entities are decoded, NBSP and other odd whitespace collapsed, category
...
4. Migrates a v1 index on the fly: URL -> slug, thumbnail URL -> `upload/<id>`, `dateVerified`
   ...
   and `og:description` re-read from the video page; if that fails they keep title, slug, date and
   the old thumbnail with no description.
